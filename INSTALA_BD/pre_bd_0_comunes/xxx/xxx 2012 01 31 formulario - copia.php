...
include('var_fichero_variables.php'); $ruta_variables_entorno = '../data/variables/'.$fichero_variables.'.php'; include("$ruta_variables_entorno"); // CARGAMOS VARIABLES DE ENTORNO PARTICULARES____________________________________________________





		$ruta_captura_variables_insert_particular = $modulo.'/034_captura_variables_insert_particular.php'; 
		if (file_exists($ruta_captura_variables_insert_particular)){include("$ruta_captura_variables_insert_particular");}

		if (strlen(trim($conf_consulta_previa_antes_formulario))>0){
		$ruta_consultas_particulares_formulario = $modulo.'/'.$conf_consulta_previa_antes_formulario; 
		include("$ruta_consultas_particulares_formulario");
		}


		include ('bd_decode_num_objeto_md5.php'); // Decodificamos el n&uacute;mero de objeto, ya que originalmente viene en MD5.

		include ('funciones/formulariosfecha.php'); // Cargamos los Men&uacute;s desplegables de las Fechas DIA | MES | A&Ntilde;O
		
		include ('funciones/decodificofecha.php'); // Aqu&iacute; est&aacute; la funci&oacute;n veo_nombremes($m) que usaremos para ver el nombre de un mes

// CARGAMOS LOS PERMISOS
$ruta_permisos = 'comunes/'.$conf_script_permisos; include("$ruta_permisos"); 


// Un Usuario puede ser que tenga permiso de INSERT, pero no de UPDATE. Con la Variable $action_original, nos aseguramos que un Usuario con permiso de INSERT, pero NO de UPDATE, pueda pulsar el Bot&oacute;n de GUARDAR tantas veces como quiera mientras est&aacute; creando originalmente el registro. Sin embargo, si despu&eacute;s (en un futuro, una vez que se salga de la pantalla original donde hace la inserci&oacute;n) intenta modificarlo ya no podr&aacute; hacerlo, hasta que se le asignen permisos de UPDATE.


if($action=="ins"){
	$maximopermitido=$var_ent_maxobjetos;
	include('bd_comprobamoscupo.php'); 
}
