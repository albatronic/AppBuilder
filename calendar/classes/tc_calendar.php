...
class tc_calendar {
    ...
    function dateAllow($from = "", $to = "", $show_not_allow = true) {
        ...
        if (is_int($time_from)) {
            ...
            if (!is_int($time_to) && !$this->year_end)
        ...
        if (is_int($time_to)) {
            ...
            if (!is_int($time_from) && !$this->year_start)
    ...
    function checkDefaultDateValid() {
        ...
        if ($this->time_allow1 && $this->time_allow2) {
            if ($default_datetime < $this->time_allow1 || $default_datetime > $this->time_allow2)
                $valid = false;
        }elseif ($this->time_allow1) {
            if ($default_datetime < $this->time_allow1)
                $valid = false;
        }elseif ($this->time_allow2) {
            if ($default_datetime > $this->time_allow2)
                $valid = false;
        }
